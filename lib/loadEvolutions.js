...
export function buildEvolutionChains(evolutionMap) {
    for (const [id, evo] of evolutionMap) {
        if (evo.evolutionChain.length === 0) {
            ...
            // Traverse backward to find the base Pokémon (starting point)
            ...
            // Traverse forward to build the full chain
            while (current) {
                chain.push({
                    id: current.id,
                    name: current.name,
                ...
                // Move forward to the next evolution stage
...
/**
 * Function to handle cases where both evolves_from and evolves_to are missing.
 * Builds chains for such Pokémon as well.
 */
export function handleMissingEvolutionChains(evolutionMap) {
    for (const [id, evo] of evolutionMap) {
        if (evo.evolutionChain.length === 0 && evo.evolvesFrom.length === 0 && evo.evolvesTo.length === 0) {
            // Single-stage Pokémon without any evolution data
            evo.evolutionChain = [{
                id: evo.id,
                name: evo.name,
                target: null,
